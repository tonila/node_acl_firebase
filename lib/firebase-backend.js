...
FirebaseBackend.prototype =  {
    ...
    */
    get: function (bucket, key, cb) {
        ...
        var self = this;
        self.fb.child(bucket).child(key).once('value', function(snap) {
    ...
    union: function (bucket, keys, cb) {
        ...
        async.parallel(fns, function(err, resArr) {
            ...
            for(var i=0,len=resArr.length;i<len;i++){
                for(var j=0,l2=resArr[i].length;j<l2;j++){
                    if(resArr[i][j]){
                        keyArrays.push(resArr[i][j]);
                    }
                }
            }
    ...
    */
    add: function (transaction, bucket, key, values) {
        ...
        transaction.push(function(cb) {
            self.fb.child(bucket).child(key).once('value', function(snap) {
                if (snap.val()) values = _.union(snap.val(), values);
                self.fb.child(bucket).child(key).set(values, function(err) {
                    cb(err);
                });
            });
        });
    ...
    del: function (transaction, bucket, keys) {
        ...
        transaction.push(function(cb) {
            ...
            for(var i=0,len=keys.length;i<len;i++){
                (function(key) {
                    fns.push(function(cb) {
                        self.fb.child(bucket).child(key).remove(function(err) {
                            cb(err);
                        });
                    });
                })(keys[i]);
    ...
    remove: function (transaction, bucket, key, values) {
        ...
        transaction.push(function(cb) {
            self.fb.child(bucket).child(key).once('value', function(snap) {
                ...
                values = removeValues(snap.val(), values);
                self.fb.child(bucket).child(key).set(values, function(err) {
                    cb(err);
                });
            });
